...
- For **private buckets**, an AWS account with permission to grant `s3:ListBucket` on the bucket and `s3:GetObject` on the objects to sync.
- For S3-compatible object stores, an HTTPS endpoint. Airbyte Cloud doesn't support insecure HTTP endpoints.

:::warning
Using cloud storage may incur egress costs. Egress refers to data transferred out of the cloud storage system, such as when you download files or access them from a different location. For details, see the [AWS S3 pricing guide](https://aws.amazon.com/s3/pricing/).
:::
...
**If you sync from a private bucket**, authenticate the connection with either an IAM user, using an **AWS Access Key ID** and **AWS Secret Access Key**, or an IAM role, using an **AWS Role ARN**. Begin by creating a policy with the necessary permissions.
...
3. Select the **JSON** tab, then paste the following JSON into the policy editor. Replace `{your-bucket-name}` with the name of your bucket.
...
:::note
Object-level permissions alone aren't sufficient. The connector lists matching files before reading them, so include both bucket-level `s3:ListBucket` permission and object-level `s3:GetObject` permission.
:::
...
##### Preserve subdirectories in file paths
...
There are no predefined streams. Each stream is based on files in your bucket that match the stream's glob pattern.
...
- **AWS Role ARN**: The [Amazon Resource Name](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_identifiers.html#identifiers-arns) of an IAM role that Airbyte can assume. In Airbyte Cloud, use your workspace ID as the role's external ID.
...
- **AWS Region**: The AWS Region where the bucket is located. If you leave this field blank, the connector determines the region automatically.
...
| :--- | :---: | :--- |
| `bucket` | Yes | Name of the S3 bucket that contains the files to replicate. |
| `streams` | Yes | List of stream configurations. Each stream must include a `name`, `globs`, and `format`. |
| `streams[].name` | Yes | Name of the Airbyte stream to create from files matching this stream configuration. |
| `streams[].globs` | Yes | List of glob patterns that identify files to replicate from the bucket. |
| `streams[].format.filetype` | Yes | File format for this stream. Valid values are `csv`, `parquet`, `avro`, `jsonl`, and `unstructured`. |
| `streams[].input_schema` | No | JSON object, serialized as a string, that defines the schema to enforce for the stream. Use `{}` to infer the schema. |
| `streams[].validation_policy` | No | How to handle records that don't match the configured or inferred schema. Valid values are `Emit Record`, `Skip Record`, and `Wait for Discover`. |
| `streams[].days_to_sync_if_history_is_full` | No | Lookback window, in days, to use for incremental syncs after the file history in state reaches its limit. |
| `streams[].schemaless` | No | Whether to skip schema validation and emit each record inside a `data` object. |
| `streams[].skip_full_check_for_parquet` | No | Whether to skip the full record parse step during source checks for Parquet streams. Defaults to `false`. |
| `aws_access_key_id` | Required for IAM user authentication | AWS access key ID for a private bucket. If you provide this field, you must also provide `aws_secret_access_key`. |
| `aws_secret_access_key` | Required for IAM user authentication | AWS secret access key for a private bucket. If you provide this field, you must also provide `aws_access_key_id`. |
| `role_arn` | Required for IAM role authentication | ARN of the IAM role that Airbyte should assume. |
| `endpoint` | No | Endpoint for an S3-compatible service. Leave blank to use Amazon S3. |
| `region_name` | No | AWS Region where the bucket is located. |
| `delivery_method.delivery_type` | No | Delivery method for replicated files. |
...
- **Header Definition**: How headers are defined. **User Provided** assumes the CSV doesn't have a header row and uses the column names you provide. **Autogenerated** assumes the CSV doesn't have a header row and generates column names as `f{i}`, where `i` is the index starting from 0. Otherwise, the connector uses the header row from the CSV file. If you want to autogenerate or provide column names for a CSV file that has a header row, set **Skip Rows Before Header** to skip the existing header row.
...
If you use **From CSV** headers, every column in the header row must have a non-empty name. For example, a header row with trailing delimiters, such as `id,name,,`, contains empty column names and causes a configuration error during discovery. Remove empty columns from the header row, provide column names manually, or use autogenerated column names.
...
- **Skip Full Check for Parquet**: Whether to skip the full record parse step during source checks. Enable this option only for large Parquet files that cause source checks to run out of memory. When this option is enabled, source checks verify that Airbyte can list and open the file, but parse errors surface during sync instead of during the check.
...
|:------------|:-----------|:----------------------------------------------------------------------------------------------------------------|:---------------------------------------------------------------------------------------------------------------------|
| 4.15.4 | 2026-05-14 | [78081](https://github.com/airbytehq/airbyte/pull/78081) | Upgrade Airbyte CDK to improve CSV header validation and column mismatch error messages |
| 4.15.3 | 2026-04-20 | [76162](https://github.com/airbytehq/airbyte/pull/76162) | Add opt-in `skip_full_check_for_parquet` stream option to skip the full parse check for Parquet files during CHECK, avoiding OOM on large files |
